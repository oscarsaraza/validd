const isArray = value =>
  (typeof value === 'object' && value !== null && !isNaN(Number(value.length)));
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!schema || Object.keys(schema).length === 0 || !schema.type) {
      ...
      if (schema.type === 'object' && data !== null) { // null is also of type 'object'
        ...
        resolveObjectPromises(validationPromises)
          .then(fieldsValidationResult => {
        ...
        if (schema.validation) {
          if (typeof schema.validation === 'function') {
            ...
            if (customValidationResult && customValidationResult.then) { // Is a promise
            ...
            } else if (customValidationResult) { // Is a validation result
              result.errors = result.errors || [];
